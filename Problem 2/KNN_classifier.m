function KNN = KNN_classifier( ...
    ...
    for i = 0:(N_te/chunk_len - 1)
        ...
        for j = 0:(N_tr/chunk_len - 1)
            ...
            [KNN_label_temp, KNN_dist_temp] = getClosestNeighborsLabels(distance, chunk_training_label, KNN_dist_temp, KNN_label_temp, idx, K);
        end
        
        % KNN_label_temp now represents the label of the K closest reference points  
        % Finds the majority vote
        m = mode(KNN_label_temp, 2);
        % Checks for tie in decision, and choose the closest value.
        m = tieBreakHandler(m, KNN_label_temp);
...
function [KNN_label_temp, KNN_dist_temp] = getClosestNeighborsLabels( ...
    distance, ...
    chunk_training_label, ...
    KNN_dist_temp, ...
    KNN_label_temp, ...
    idx, ...
    K)
...
function m = tieBreakHandler(m, KNN_label_temp)
